...
const countEmployees = function() {
  ...
  enterprise.departments.forEach((dep, index) => { // обход отделов для счетчика
    ...
    if (count == 0) console.log(`${enterprise.name}: Нет сотрудников`); 
      else console.log(`${enterprise.name} cотрудников ${count}`);
    
  enterprise.departments.forEach((dep, index) => { // обход отделов для вывода
      count= count+dep.employees_count;
      if (count == 0) return console.log(`- ${dep.name}: Нет сотрудников`); 
